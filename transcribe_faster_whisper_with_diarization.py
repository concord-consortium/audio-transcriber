...
ALLOWED_AUDIO_FORMATS = {'.mp3', '.wav', '.m4a', '.ogg', '.flac', '.mp4'}
...
def convert_to_wav(audio_file_path, temp_wav_file):
    ...
    try:
        ...
        sys.stderr.write(f"Attempting to load audio file: {audio_file_path}\n")
        
        # Try to load the audio file with explicit format
        try:
            audio = AudioSegment.from_file(audio_file_path, format="mp4")
        except Exception as e:
            sys.stderr.write(f"Failed to load with explicit mp4 format, trying auto-detect: {str(e)}\n")
            audio = AudioSegment.from_file(audio_file_path)
        
        if audio is None or len(audio) == 0:
            raise ValueError("No audio content found in the file")
        ...
        audio.export(wav_file_path, format="wav", parameters=["-ac", "1"])  # Convert to mono
